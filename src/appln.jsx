...
const MatrixBackground = ({ intensity = 0.6 }) => {
  ...
  useEffect(() => {
    ...
    const fontSize = 16; // clean + readable
    ...
    // Colors
    const bgFade = 0.075; // subtle trail
    const glow = 0.25; // subtle glow for cleanliness

    const draw = () => {
      // translucent fade rect for trails
      ...
      for (let i = 0; i < drops.length; i++) {
        // randomness scaled by intensity for cleanliness
        if (Math.random() > (0.975 + (1 - intensity) * 0.02)) continue;

        const text = chars[Math.floor(Math.random() * charLen)];
        const x = i * fontSize;
...
// --- Small UI Helpers (loaders etc.) ---
const LoaderSpinner = ({ size = 20 }) => (
  <span className="spinner" style={{ width: size, height: size }} aria-hidden="true" />
);

const Ellipsis = () => <span className="ellipsis"><span></span><span></span><span></span></span>;
...
function Appln() {
  ...
  if (authState !== 'LOGGED_IN') {
    return (
      <>
        <MatrixBackground intensity={0.55} />
            ...
            <div className="auth-columns">
              <div className="auth-column">
                ...
                <form onSubmit={handleLogin} className="auth-form">
                  ...
                  <button type="submit" className="btn-primary" disabled={isLoading}>
                    {isLoading ? (<><LoaderSpinner /> <span className="btn-gap">Logging in</span></>) : 'Login'}
                  </button>
                </form>
              </div>
              ...
              <div className="auth-column">
                ...
                {authState === 'LOGGED_OUT' && (
                  <button onClick={handleWalletConnect} className="btn-connect" disabled={isLoading}>
                    {isLoading ? (<><LoaderSpinner /> <span className="btn-gap">Connecting</span></>) : 'Connect & Register Wallet'}
                  </button>
                )}
                {authState === 'REGISTERING' && (
                  <form onSubmit={handleRegister} className="auth-form">
                    ...
                    <button type="submit" className="btn-primary" disabled={isLoading}>
                      {isLoading ? (<><LoaderSpinner /> <span className="btn-gap">Creating Account</span></>) : 'Create Account'}
                    </button>
                  </form>
                )}
              </div>
          ...
          {isLoading && (
            ...
            >
              <div className="overlay-card">
                ...
                <div className="overlay-text">Working on it <Ellipsis /></div>
              </div>
            </motion.div>
          )}
        </AnimatePresence>
    ...
    <>
      ...
      <div className="main-container">
        ...
        <div className="content-grid">
          ...
          >
            <motion.div className="card glass" whileHover={{ y: -2 }} transition={{ type: 'spring', stiffness: 300, damping: 20 }}>
              <h3>Your Balances</h3>
            ...
            <motion.div className="card glass" whileHover={{ y: -2 }} transition={{ type: 'spring', stiffness: 300, damping: 20 }}>
              ...
              <div className="form-group">
                ...
                <button onClick={createAgreement} disabled={isLoading}>
                  {isLoading ? (<><LoaderSpinner /> <span className="btn-gap">Creating</span></>) : 'Create Agreement'}
                </button>
              </div>
            </motion.div>
          </motion.div>
                    ...
                    >
                      ...
                      <div className="item-actions">
                        {agg.status === 'Created' && agg.depositor.toLowerCase() === account?.toLowerCase() && (
                          <button onClick={() => handleAction(agg, 'Fund')} disabled={isLoading}>
                            {isLoading ? (<><LoaderSpinner /> <span className="btn-gap">Funding</span></>) : 'Fund Escrow'}
                          </button>
                        )}
                        {agg.status === 'Funded' && agg.arbiter.toLowerCase() === account?.toLowerCase() && (
                          <button onClick={() => handleAction(agg, 'Release')} disabled={isLoading}>
                            {isLoading ? (<><LoaderSpinner /> <span className="btn-gap">Releasing</span></>) : 'Release Funds'}
                          </button>
                        )}
                        {agg.status === 'Expired' && (
      ...
      <AnimatePresence>
        {isLoading && (
          ...
          >
            <div className="overlay-card">
              <LoaderSpinner size={36} />
              <div className="overlay-text">Processing <Ellipsis /></div>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
